...
Options:
  ...
  summary [n]   Générer un résumé des alertes des n derniers jours (défaut: 7)
  clean [n]     Nettoyer les alertes plus anciennes que n jours (défaut: 30)
  block <ip>    Bloquer manuellement une adresse IP
  unblock <ip>  Débloquer une adresse IP
  list-blocked  Lister les IPs bloquées
  menu          Lancer l'interface interactive (whiptail/dialog)
